...
public class Cliente {
	...
	public void startCliente(String servidor_seleccionado) throws RemoteException, NotBoundException {
		if(servidor_seleccionado == "Chile") {
			Registry registry = LocateRegistry.getRegistry("localhost", 5000);
			server = (InterfazDeServer) registry.lookup("server");
			System.out.println("Cliente conectado en Chile!!");
		} else if(servidor_seleccionado == "USA") {
			Registry registry = LocateRegistry.getRegistry("localhost", 5001);
			server = (InterfazDeServer) registry.lookup("server");
			System.out.println("Cliente conectado en USA!!");
		}
		else {
			System.out.println("Hubo un problema iniciando el cliente");
		}	
	}
	...
	Persona newPersona(String nombre, String apellido, int creditos, String tipo_contrato) throws RemoteException {
		System.out.println("Enviando solicitud al server...");
		return server.newPersona(nombre, apellido, creditos, tipo_contrato);
	}
